...
def open_messagebox():
    ...
    def filemenu():
        folder = resolve_path("\\Saves\\OneVPCH")
    ...
    def NEWGAME():
        global Player1
        msg1 = CTkInputDialog(title="Player name", text="Enter player name")
...
def load_game(loadedfile):
    ...
    global turn2
    global ButtonPlayer1
    Load = open(resolve_path(f"\\Saves\\OneVPCH\\{loadedfile}.txt"), "r")
    Mode=Load.readline()
...
def save_game():
    ...
    F=open(resolve_path(f"\\Saves\\OneVPCH\\{FileName}.txt"), "w")
