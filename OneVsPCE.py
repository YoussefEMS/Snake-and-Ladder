...
def open_messagebox():
    ...
    def filemenu():
        folder = resolve_path("\\Saves\\OneVPCE")
    ...
    def NEWGAME():
        ...
        while (Player1.isalpha() == False):
            ...
            Player1=msg1.get_input()
        ButtonPlayer1=CTkButton(root, text=Player1, height=50, width=100 ,hover_color="white", fg_color='orange',state=State1, command=diceimg1).place(x=920,y=450)    
        #Show Scores
        Player1Name=CTkLabel(root, text=Player1, font=("Arial", 20), fg_color="orange",text_color="white", anchor="center", width=60, height=30).place(x=1200, y=170)
        Player2Name=CTkLabel(root, text=Player2, font=("Arial", 20), fg_color="black",text_color="white", anchor="center", width=60, height=30).place(x=1260, y=170)
    ...
    def rules():
        CTkMessagebox(title="First Rule", message="We are about to play the Game. But at first let me tell you that at first You and the PC both will roll the dice and the one who will get a higher number will play first",font=("Times New Roma", 20 , "bold"), icon=resolve_path("\Icon\GreenIntroduction.ico"), fg_color= "white", bg_color="white",text_color="black",title_color="black", icon_size=(250,300),width=320,height=210)
...
GameBoard=CTkImage(light_image=Image.open(resolve_path("//Board//GameBoardE.png")), size=(600,600))
...
def diceimg1():
    ...
    global sixcount1
    ButtonPlayer1=CTkButton(root, text=Player1, height=50, width=100 ,hover_color="white", fg_color='orange',state=State1, command=diceimg1).place(x=920,y=450)
    path1=random.randint(1,6)
    dicephoto=DiceList[path1-1]
    DiceImage= CTkImage(light_image=Image.open(dicephoto), size=(120,120))
    label1.place(x=900, y=250)
    label1.configure(image=DiceImage, text=None, width=120, height=120)
    time.sleep(0.03)
    if turn1>0:
        ...
        if path1==6:
            ...
            sixcount1=0
        if sixcount1>0 and sixcount1<3:
            Flag1=TRUE
            State1=NORMAL
            ButtonPlayer1=CTkButton(root, text=Player1, height=50, width=100 ,hover_color="white", fg_color='orange',state=State1, command=diceimg1).place(x=920,y=450)
        elif sixcount1>=4 or sixcount1==0:
            Flag1=FALSE
            State1=DISABLED
            ButtonPlayer1=CTkButton(root, text=Player1, height=50, width=100 ,hover_color="white", fg_color='orange',state=State1, command=diceimg1).place(x=920,y=450)
    else:
...
def diceimg2():
    ...
    ButtonPlayer1=CTkButton(root, text=Player1, height=50, width=100 ,hover_color="white", fg_color='orange',state=State1, command=diceimg1).place(x=920,y=450)
    Position_Diff=0
    path2=random.randint(1,6)
    dicephoto=DiceList[path2-1]
    DiceImage= CTkImage(light_image=Image.open(dicephoto), size=(120,120))
    label1.place(x=900, y=250)
    label1.configure(image=DiceImage, text=None, width=120, height=120)
    if turn2>0:
        ...
        if sixcount2>0 and sixcount2<4:
            State1=DISABLED
            ButtonPlayer1=CTkButton(root, text=Player1, height=50, width=100 ,hover_color="white", fg_color='orange',state=State1, command=diceimg1).place(x=920,y=450)
            root.after(700,diceimg2)
        else:
    ...
    if coin2_position==0:
        ...
        if coin2_position in PositionDict.keys():
            ...
            if coin2_position==100:
                State1=DISABLED
                ButtonPlayer1=CTkButton(root, text=Player1, height=50, width=100 ,hover_color="black", fg_color='orange',state=State1, command=diceimg1).place(x=920,y=450)
...
def PlayerTurn():
    ...
    if GlobalPath1>GlobalPath2:
        State1=NORMAL
        turn1=20
        CTkMessagebox(title='Game start',title_color="black", message=f"You won. You will start the game!!", text_color= "Black", icon=resolve_path("/Icon/OrangeIntroduction.ico"),icon_size=(200,250), bg_color="white", fg_color="white", width=300, height=250)

    if GlobalPath2>GlobalPath1:
        State1=DISABLED
        State2=NORMAL
        CTkMessagebox(title='Game start',title_color="black", message=f"{Player2} won. The PC will start game!!", text_color= "Black", icon=resolve_path("/Icon/OrangeIntroduction.ico"),icon_size=(200,250), bg_color="white", fg_color="white", width=300, height=250)
        root.after(1000,diceimg2)
    elif GlobalPath1==GlobalPath2:
        turn1=0
        turn2=0
        CTkMessagebox(title='Game start',title_color="black", message=f"It is draw situation. You will have to roll the dice again", text_color= "Black", icon=resolve_path("/Icon/OrangeIntroduction.ico"),icon_size=(200,250), bg_color="white", fg_color="white", width=300, height=250)
...
progressbar1.set(0)
progressbar1.place(x = 920 , y = 400)
...
def load_game(loadedfile):
    ...
    global turn2
    global ButtonPlayer1
    Load = open(resolve_path(f"\\Saves\\OneVPCE\\{loadedfile}.txt"), "r")
    Mode=Load.readline()
    ...
    State1=Literal[f'{state1value}']
    state2value=Load.readline()
    state2value=state2value.rstrip()
    State2=Literal[f"{state2value}"]
...
def save_game():
    ...
    F=open(resolve_path(f"\\Saves\\OneVPCE\\{FileName}.txt"), "w")
